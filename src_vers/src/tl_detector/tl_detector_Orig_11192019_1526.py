...
diff_cpsl_lb = 0 # lower bound
diff_cpsl_ub = 35 # upper bound
...
class TLDetector(object):
    def __init__(self):
        ...
        self.diff_cpsl = None # waypoint distance/difference between car current location and traffic light stop line
    ...
    def get_light_state(self, light):
        ...
        # file_name = 'imgs/'+my_ct2+'.jpg'
        # cv2.imwrite(file_name, cv_image)

        # from above code block it was determined optimum distance is about 35-10 waypoints 
        # optimum distance: between car current waypoint/pose and traffic light stop line waypoint

        rospy.logwarn("diff_cpsl: {0}".format(self.diff_cpsl))
        # rospy.logwarn("odc_count: {0}".format(self.odc_count))
        ...
        if self.diff_cpsl >= diff_cpsl_lb and self.diff_cpsl <= diff_cpsl_ub:
            ...
            self.classification_prev = classification

            rospy.logwarn("Light State (Ground Truth): {0}".format(light.state))
            rospy.logwarn("Light State (OD Output): {0}".format(classification))

        else:

            classification = self.classification_prev
